...
"""
Sets up the TrustGraph hardware security demo workspace with knowledge graph
...
HWSEC_DIR = os.path.join(DATASETS_DIR, "hwsec")
...
QUERY_FILES = [
    ("hwsec-queries.json", "hwsec SPARQL"),
]

TOOL_FILES = {
    "hwsec-query": "hwsec-tool.json",
}
...
def upload_ontology(api):
    ...
    path = os.path.join(HWSEC_DIR, "hwsec-ontology.json")
    with open(path) as f:
        value = f.read()
    put_config(api, "ontology", "hwsec-ontology", value)
...
def load_knowledge(api, flow="default"):
    ...
    document_id = "urn:doc:hwsec"
    collection = "default"

    bulk = api.bulk()

    for filename in all_files:
        path = os.path.join(HWSEC_DIR, filename)
