...
"""
Sets up the TrustGraph risk management demo workspace with knowledge graph
...
RISK_DIR = os.path.join(DATASETS_DIR, "risk")
...
QUERY_FILES = [
    ("risk-queries.json", "SPARQL"),
    ("risk-graph-rag-queries.json", "GraphRAG"),
    ("risk-agent-queries.json", "agent"),
]

TOOL_FILES = {
    "risk-query": "risk-tool.json",
}
...
def load_knowledge(api, flow="default"):
    ...
    document_id = "urn:doc:risk"
    collection = "default"

    bulk = api.bulk()

    for filename in all_files:
        path = os.path.join(RISK_DIR, filename)
